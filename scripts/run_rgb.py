...
# 1️⃣ 结果目录
# ------------------------
save_dir = os.path.join(RESULT_DIR, "rgb_scoring_fixed")  # 新目录
...
# ============================================================
print("\n📊 计算相似度矩阵...")
sim_to_gallery = cosine_similarity(query_feats, gallery_feats)  # [Q, G]
...
scores_max = sim_to_gallery.max(axis=1)                    # Max
scores_top5 = score_topk(sim_to_gallery, k=5)              # Top-5 Mean
scores_margin = score_margin(sim_to_gallery)                # Margin
scores_energy = score_energy(sim_to_gallery)                # Energy 🔥 不加负号！
...
plt.tight_layout()
plot_path = os.path.join(save_dir, "scoring_comparison_rgb.png")
plt.savefig(plot_path, dpi=200, bbox_inches="tight")
plt.show()
print(f"对比图保存至: {plot_path}")

# ============================================================
# 1️⃣1️⃣ 保存结果
# ============================================================
np.savez(
    os.path.join(save_dir, f"rgb_scoring_comparison_seed{SEED}.npz"),
